...
def test_unique_uri_validator_create(db):
    ViewUniqueURIValidator()({'uri_prefix': settings.DEFAULT_URI_PREFIX, 'key': 'test'})


def test_unique_uri_validator_create_error(db):
    with pytest.raises(ValidationError):
        ViewUniqueURIValidator()({'uri_prefix': settings.DEFAULT_URI_PREFIX, 'key': View.objects.last().key})


def test_unique_uri_validator_update(db):
    view = View.objects.first()

    ViewUniqueURIValidator(view)({'uri_prefix': view.uri_prefix, 'key': view.key})
...
def test_unique_uri_validator_update_error(db):
    ...
    with pytest.raises(ValidationError):
        ViewUniqueURIValidator(view)({'uri_prefix': view.uri_prefix, 'key': View.objects.last().key})
...
def test_unique_uri_validator_serializer_create_error(db):
    ...
    with pytest.raises(RestFameworkValidationError):
        validator({'uri_prefix': settings.DEFAULT_URI_PREFIX, 'key': View.objects.last().key})
...
def test_unique_uri_validator_serializer_update(db):
    ...
    validator({'uri_prefix': view.uri_prefix, 'key': view.key})
...
def test_unique_uri_validator_serializer_update_error(db):
    ...
    with pytest.raises(RestFameworkValidationError):
        validator({'uri_prefix': view.uri_prefix, 'key': View.objects.last().key})

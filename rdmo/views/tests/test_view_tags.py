...
@pytest.fixture
def context(db):
    return Context({'project': ProjectWrapper(Project.objects.get(pk=project_pk))})
...
def test_get_set_values(context, values):
    ...
    for value_set in get_sets(context, 'set'):
        assertListEqual(
            get_set_values(context, value_set, path),
...
def test_get_set_value(context, values):
    ...
    for value_set in get_sets(context, 'set'):
        assert (
            get_set_value(context, value_set, path)['id']

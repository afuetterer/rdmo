...
def test_unique_uri_validator_create(db):
    TaskUniqueURIValidator()({'uri_prefix': settings.DEFAULT_URI_PREFIX, 'key': 'test'})


def test_unique_uri_validator_create_error(db):
    with pytest.raises(ValidationError):
        TaskUniqueURIValidator()({'uri_prefix': settings.DEFAULT_URI_PREFIX, 'key': Task.objects.last().key})


def test_unique_uri_validator_update(db):
    task = Task.objects.first()

    TaskUniqueURIValidator(task)({'uri_prefix': task.uri_prefix, 'key': task.key})
...
def test_unique_uri_validator_update_error(db):
    ...
    with pytest.raises(ValidationError):
        TaskUniqueURIValidator(task)({'uri_prefix': task.uri_prefix, 'key': Task.objects.last().key})
...
def test_unique_uri_validator_serializer_create_error(db):
    ...
    with pytest.raises(RestFameworkValidationError):
        validator({'uri_prefix': settings.DEFAULT_URI_PREFIX, 'key': Task.objects.last().key})
...
def test_unique_uri_validator_serializer_update(db):
    ...
    validator({'uri_prefix': task.uri_prefix, 'key': task.key})
...
def test_unique_uri_validator_serializer_update_error(db):
    ...
    with pytest.raises(RestFameworkValidationError):
        validator({'uri_prefix': task.uri_prefix, 'key': Task.objects.last().key})

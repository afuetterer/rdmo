...
def test_create(db):
    SectionLockedValidator()({'catalog': Catalog.objects.first(), 'locked': False})


def test_create_locked(db):
    SectionLockedValidator()({'catalog': Catalog.objects.first(), 'locked': True})


def test_update(db):
    section = Section.objects.first()

    SectionLockedValidator(section)({'catalog': section.catalog, 'locked': False})
...
def test_update_error(db):
    ...
    with pytest.raises(ValidationError):
        SectionLockedValidator(section)({'catalog': section.catalog, 'locked': True})
...
def test_serializer_create(db):
    ...
    validator({'catalog': Catalog.objects.first(), 'locked': False})
...
def test_serializer_create_locked(db):
    ...
    validator({'catalog': Catalog.objects.first(), 'locked': True})
...
def test_serializer_update(db):
    ...
    validator({'catalog': section.catalog, 'locked': False})
...
def test_serializer_update_error(db):
    ...
    with pytest.raises(RestFameworkValidationError):
        validator({'catalog': section.catalog, 'locked': True})

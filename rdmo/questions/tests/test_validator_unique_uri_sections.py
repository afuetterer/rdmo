...
def test_unique_uri_validator_create(db):
    SectionUniqueURIValidator()(
        {'uri_prefix': settings.DEFAULT_URI_PREFIX, 'key': 'test', 'catalog': Catalog.objects.first()}
    )
...
def test_unique_uri_validator_create_error(db):
    ...
    with pytest.raises(ValidationError):
        SectionUniqueURIValidator()(
            {'uri_prefix': settings.DEFAULT_URI_PREFIX, 'key': catalog.sections.first().key, 'catalog': catalog}
        )
...
def test_unique_uri_validator_update(db):
    ...
    SectionUniqueURIValidator(section)(
        {'uri_prefix': section.uri_prefix, 'key': section.key, 'catalog': section.catalog}
    )
...
def test_unique_uri_validator_update_error(db):
    ...
    with pytest.raises(ValidationError):
        SectionUniqueURIValidator(section)(
            {'uri_prefix': section.uri_prefix, 'key': section.catalog.sections.last().key, 'catalog': section.catalog}
        )
...
def test_unique_uri_validator_serializer_create(db):
    ...
    validator({'uri_prefix': settings.DEFAULT_URI_PREFIX, 'key': 'test', 'catalog': Catalog.objects.first()})
...
def test_unique_uri_validator_serializer_create_error(db):
    ...
    with pytest.raises(RestFameworkValidationError):
        validator({'uri_prefix': settings.DEFAULT_URI_PREFIX, 'key': catalog.sections.last().key, 'catalog': catalog})
...
def test_unique_uri_validator_serializer_update(db):
    ...
    validator({'uri_prefix': section.uri_prefix, 'key': section.key, 'catalog': section.catalog})

...
def test_unique_uri_validator_create(db):
    CatalogUniqueURIValidator()({'uri_prefix': settings.DEFAULT_URI_PREFIX, 'key': 'test'})


def test_unique_uri_validator_create_error(db):
    with pytest.raises(ValidationError):
        CatalogUniqueURIValidator()({'uri_prefix': settings.DEFAULT_URI_PREFIX, 'key': Catalog.objects.get(pk=2).key})


def test_unique_uri_validator_update(db):
    catalog = Catalog.objects.get(pk=1)

    CatalogUniqueURIValidator(catalog)({'uri_prefix': catalog.uri_prefix, 'key': catalog.key})
...
def test_unique_uri_validator_update_error(db):
    ...
    with pytest.raises(ValidationError):
        CatalogUniqueURIValidator(catalog)({'uri_prefix': catalog.uri_prefix, 'key': Catalog.objects.get(pk=2).key})
...
def test_unique_uri_validator_serializer_create_error(db):
    ...
    with pytest.raises(RestFameworkValidationError):
        validator({'uri_prefix': settings.DEFAULT_URI_PREFIX, 'key': Catalog.objects.get(pk=2).key})
...
def test_unique_uri_validator_serializer_update(db):
    ...
    validator({'uri_prefix': catalog.uri_prefix, 'key': catalog.key})
...
def test_unique_uri_validator_serializer_update_error(db):
    ...
    with pytest.raises(RestFameworkValidationError):
        validator({'uri_prefix': catalog.uri_prefix, 'key': Catalog.objects.get(pk=2).key})

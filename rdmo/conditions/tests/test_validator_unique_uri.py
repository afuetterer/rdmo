...
def test_unique_uri_validator_create(db):
    ConditionUniqueURIValidator()({'uri_prefix': settings.DEFAULT_URI_PREFIX, 'key': 'test'})


def test_unique_uri_validator_create_error(db):
    with pytest.raises(ValidationError):
        ConditionUniqueURIValidator()({'uri_prefix': settings.DEFAULT_URI_PREFIX, 'key': Condition.objects.last().key})


def test_unique_uri_validator_update(db):
    condition = Condition.objects.first()

    ConditionUniqueURIValidator(condition)({'uri_prefix': condition.uri_prefix, 'key': condition.key})
...
def test_unique_uri_validator_update_error(db):
    ...
    with pytest.raises(ValidationError):
        ConditionUniqueURIValidator(condition)(
            {'uri_prefix': condition.uri_prefix, 'key': Condition.objects.last().key}
        )
...
def test_unique_uri_validator_serializer_create_error(db):
    ...
    with pytest.raises(RestFameworkValidationError):
        validator({'uri_prefix': settings.DEFAULT_URI_PREFIX, 'key': Condition.objects.last().key})
...
def test_unique_uri_validator_serializer_update(db):
    ...
    validator({'uri_prefix': condition.uri_prefix, 'key': condition.key})
...
def test_unique_uri_validator_serializer_update_error(db):
    ...
    with pytest.raises(RestFameworkValidationError):
        validator({'uri_prefix': condition.uri_prefix, 'key': Condition.objects.last().key})

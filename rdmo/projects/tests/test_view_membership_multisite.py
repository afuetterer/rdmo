...
add_membership_permission_map = {'owner': [1, 2, 3, 4, 5], 'api': [1, 2, 3, 4, 5], 'site': [1, 2, 3, 4, 5]}
...
def test_get_invite_email_project_path_function(
    db, client, username, password, project_id, membership_role, site_domain, multisite_setting
):
    ...
    foo_user, _created = get_user_model().objects.get_or_create(
        username=foo_username, email=foo_email, password=foo_username
    )
...
def test_invite_email_project_path_email_body(
    ...
    foo_email = f'{foo_username}@{site_domain}'
    foo_user, _created = get_user_model().objects.get_or_create(
        username=f'{site_domain}-user', email=foo_email, password=foo_username
    )
    foo_user.role.member.set([foo_site])
    project = Project.objects.get(pk=project_id)

    url = reverse('membership_create', args=[project_id])
    data = {'username_or_email': foo_email, 'role': membership_role}

...
def test_validator_update_error(db):
    ...
    with pytest.raises(ValidationError):
        AttributeUniqueURIValidator(attribute)(
            {'uri_prefix': attribute.uri_prefix, 'key': 'textarea', 'parent': attribute.parent}
        )
...
def test_validator_serializer_update(db):
    ...
    validator({'uri_prefix': attribute.uri_prefix, 'key': 'test', 'parent': attribute.parent})
...
def test_validator_serializer_update_error(db):
    ...
    with pytest.raises(RestFameworkValidationError):
        validator({'uri_prefix': attribute.uri_prefix, 'key': 'textarea', 'parent': attribute.parent})

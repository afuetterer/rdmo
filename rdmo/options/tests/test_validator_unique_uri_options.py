...
def test_unique_uri_validator_create(db):
    OptionUniqueURIValidator()(
        {'uri_prefix': settings.DEFAULT_URI_PREFIX, 'key': 'test', 'optionset': OptionSet.objects.first()}
    )
...
def test_unique_uri_validator_create_error(db):
    ...
    with pytest.raises(ValidationError):
        OptionUniqueURIValidator()(
            {'uri_prefix': settings.DEFAULT_URI_PREFIX, 'key': optionset.options.first().key, 'optionset': optionset}
        )
...
def test_unique_uri_validator_update(db):
    ...
    OptionUniqueURIValidator(option)(
        {'uri_prefix': option.uri_prefix, 'key': option.key, 'optionset': option.optionset}
    )
...
def test_unique_uri_validator_update_error(db):
    ...
    with pytest.raises(ValidationError):
        OptionUniqueURIValidator(option)(
            {'uri_prefix': option.uri_prefix, 'key': option.optionset.options.last().key, 'optionset': option.optionset}
        )
...
def test_unique_uri_validator_serializer_create(db):
    ...
    validator({'uri_prefix': settings.DEFAULT_URI_PREFIX, 'key': 'test', 'optionset': OptionSet.objects.first()})
...
def test_unique_uri_validator_serializer_create_error(db):
    ...
    with pytest.raises(RestFameworkValidationError):
        validator(
            {'uri_prefix': settings.DEFAULT_URI_PREFIX, 'key': optionset.options.last().key, 'optionset': optionset}
        )
...
def test_unique_uri_validator_serializer_update(db):
    ...
    validator({'uri_prefix': option.uri_prefix, 'key': option.key, 'optionset': option.optionset})

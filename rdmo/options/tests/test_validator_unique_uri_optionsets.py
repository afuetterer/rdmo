...
def test_unique_uri_validator_create(db):
    OptionSetUniqueURIValidator()({'uri_prefix': settings.DEFAULT_URI_PREFIX, 'key': 'test'})


def test_unique_uri_validator_create_error(db):
    with pytest.raises(ValidationError):
        OptionSetUniqueURIValidator()({'uri_prefix': settings.DEFAULT_URI_PREFIX, 'key': OptionSet.objects.last().key})


def test_unique_uri_validator_update(db):
    optionset = OptionSet.objects.first()

    OptionSetUniqueURIValidator(optionset)({'uri_prefix': optionset.uri_prefix, 'key': optionset.key})
...
def test_unique_uri_validator_update_error(db):
    ...
    with pytest.raises(ValidationError):
        OptionSetUniqueURIValidator(optionset)(
            {'uri_prefix': optionset.uri_prefix, 'key': OptionSet.objects.last().key}
        )
...
def test_unique_uri_validator_serializer_create_error(db):
    ...
    with pytest.raises(RestFameworkValidationError):
        validator({'uri_prefix': settings.DEFAULT_URI_PREFIX, 'key': OptionSet.objects.last().key})
...
def test_unique_uri_validator_serializer_update(db):
    ...
    validator({'uri_prefix': optionset.uri_prefix, 'key': optionset.key})
...
def test_unique_uri_validator_serializer_update_error(db):
    ...
    with pytest.raises(RestFameworkValidationError):
        validator({'uri_prefix': optionset.uri_prefix, 'key': OptionSet.objects.last().key})
